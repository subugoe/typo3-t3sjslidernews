 ...
 */
class tx_t3sjslidernews_pi1 extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin
{
    public $prefixId = 'tx_t3sjslidernews_pi1';
    public $scriptRelPath = 'pi1/class.tx_t3sjslidernews_pi1.php';
    public $extKey = 't3s_jslidernews';
    public $pi_checkCHash = true;

    public $sliderWidth = 700;
    public $sliderHeight = 300;
    ...
    public $DAM = false;
    public $extConf = [];
    public $variant = false; // option in sliderstyle 2 & 3 (slide out navigation)
    public $iframeOverlay = false;
    public $inlineOverlay = false;
    public $readmore = 0;
    ...
    public function main(string $content, array $conf): string
    {
        ...
        switch ($this->conf['contentStyle']) {
            case 'tt_news':
                if (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('tt_news') == 'TRUE') {
            ...
            foreach ($data as $lang => $value) {
                foreach ($value as $key => $val) {
                    if (is_array($val)) {
                        ...
                        if (is_numeric($this->conf[$key])) {
                            $this->conf[$key] = intval($this->conf[$key]);
                        }
                    }
                }
            }
    ...
    protected function getSliderNewsRecords(): array
    {
        ...
        $_tt_news_obj = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('tx_ttnews');
        $_tt_news_obj->cObj = &$this->cObj;
        $newsConf = [];
        // SELECT
        $newsConf['selectFields'] = $this->pi_prependFieldsWithTable('tt_news',
            'uid,pid,title,datetime,image,imagecaption,imagealttext,imagetitletext,short,bodytext,author,author_email,category,type,ext_url,page,sys_language_uid,archivedate');
        if (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('rgnewsce')) {
            $newsConf['selectFields'] .= ',tt_news.tx_rgnewsce_ce';
        }
        if (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('dam_ttnews')) {
            $newsConf['selectFields'] .= ',tt_news.tx_damnews_dam_images';
        }
        if (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('rgmediaimagesttnews')) {
            $newsConf['selectFields'] .= ',tt_news.tx_rgmediaimages_config';
        }
        if ($this->extConf['ownMarker_1'] && (strpos($this->extConf['ownMarker_1'], 'tt_news.') !== false)) {
            $newsConf['selectFields'] .= ',' . trim($this->extConf['ownMarker_1']);
        }
        if ($this->extConf['ownMarker_2'] && (strpos($this->extConf['ownMarker_2'], 'tt_news.') !== false)) {
            $newsConf['selectFields'] .= ',' . trim($this->extConf['ownMarker_2']);
        }
        // WHERE
        $newsConf['where'] = ' 1=1';
        ...
        if ($this->extConf['recLevel']) {
            $newsConf['pidInList'] = $this->pi_getPidList($newsConf['pidInList'], $this->extConf['recLevel']);
        }
        // sliderCat
        ...
        if ($this->conf['ascDesc'] == 1) {
            $newsConf['orderBy'] = 'tt_news.' . $this->conf['ascDescField'] . ' ASC';
        }
        if ($this->conf['ascDesc'] == 2) {
            $newsConf['orderBy'] = 'RAND()';
        }
        // newsArchive
        ...
        while ($newsRow = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($newsRes)) {
            ...
            $newsRecords[$nUid]['image'] = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',',
                $newsRecords[$nUid]['image']);
            $newsRecords[$nUid]['image'] = $newsRecords[$nUid]['image'][0];

            if (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('rgnewsce') == 'TRUE' && $newsRecords[$nUid]['tx_rgnewsce_ce'] && !$newsRecords[$nUid]['image']) {
            ...
            if ($_tt_news_setup['useHRDates'] && $_tt_news_setup['useHRDatesSingle']) {
                if ($_tt_news_setup['useHRDatesSingleWithoutDay']) {
                    $newsRecords[$nUid]['addParam'] = '&tx_ttnews[year]=' . date('Y',
                            $newsRecords[$nUid]['datetime']) . '&tx_ttnews[month]=' . date('m',
                            $newsRecords[$nUid]['datetime']) . '&tx_ttnews[tt_news]=' . $newsRecords[$nUid]['uid'];
                } else {
                    $newsRecords[$nUid]['addParam'] = '&tx_ttnews[year]=' . date('Y',
                            $newsRecords[$nUid]['datetime']) . '&tx_ttnews[month]=' . date('m',
                            $newsRecords[$nUid]['datetime']) . '&tx_ttnews[day]=' . date('d',
                            $newsRecords[$nUid]['datetime']) . '&tx_ttnews[tt_news]=' . $newsRecords[$nUid]['uid'];
                }
            } else {
            ...
            if ($GLOBALS['TSFE']->sys_language_content) {
                ...
                if (!$newsRecords[$nUid] && !$ll) {
                    unset($newsRecords[$nUid]);
                }
            }
            // if an image is required ... unset all records without an image
            if ($this->conf['requireImage'] === 0 && !$newsRecords[$nUid]['image']) {
                unset($newsRecords[$nUid]);
            }
        }
    ...
    protected function getSliderContentRecords(): array
    {
        ...
        if ($this->conf['contentPages']) {
            $pageUids = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $this->conf['contentPages']);
            foreach ($pageUids as $pageUid) {
                ...
                while ($rowCe = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($resPages)) {
                    $pageCeUids .= ',' . $rowCe['uid'];
                }
        ...
        if ($pageCeUids) {
            if ($this->conf['contentElement']) {
                $this->conf['contentElement'] = \TYPO3\CMS\Core\Utility\GeneralUtility::uniqueList($this->conf['contentElement'] . $pageCeUids);
            } else {
                $this->conf['contentElement'] = substr($pageCeUids, 1);
            }
        }
        $select = $this->pi_prependFieldsWithTable('tt_content',
            'uid,pid,header,bodytext,image,subheader,header_link,imagecaption,date,altText,titleText,sys_language_uid,CType,pi_flexform,section_frame');
        if ($this->extConf['ownMarker_1'] && (strpos($this->extConf['ownMarker_1'], 'tt_content.') !== false)) {
            $select .= ',' . trim($this->extConf['ownMarker_1']);
        }
        if ($this->extConf['ownMarker_2'] && (strpos($this->extConf['ownMarker_2'], 'tt_content.') !== false)) {
            $select .= ',' . trim($this->extConf['ownMarker_2']);
        }
        ...
        foreach ($rows as $key => $row) {
            ...
            if ($GLOBALS['TSFE']->sys_language_content) {
                $contentRecords[$key] = $GLOBALS['TSFE']->sys_page->getRecordOverlay('tt_content',
                    $contentRecords[$key], $GLOBALS['TSFE']->sys_language_content,
                    $GLOBALS['TSFE']->sys_language_contentOL, '');
                $contentRecords[$key]['uid'] = $contentRecords[$key]['_LOCALIZED_UID'] ? $contentRecords[$key]['_LOCALIZED_UID'] : $contentRecords[$key]['pid'];
            }

            if (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('rgmediaimages')) {
                if (strpos($contentRecords[$key]['image'],
                        '.flv') !== false || strpos($contentRecords[$key]['image'],
                        '.pdf') !== false || strpos($contentRecords[$key]['image'],
                        '.ai') !== false || strpos($contentRecords[$key]['image'],
                        '.swf') !== false || strpos($contentRecords[$key]['image'],
                        '.mp3') !== false || strpos($contentRecords[$key]['image'], '.rgg') !== false
                ) {
            ...
            if ($GLOBALS['TSFE']->sys_language_content) {
                ...
                $contentRecords[$key] = $GLOBALS['TSFE']->sys_page->getRecordOverlay('tt_content',
                    $contentRecords[$key], $GLOBALS['TSFE']->sys_language_content, $OLmode);
                if (!$contentRecords[$key] && !$ll) {
                    unset($contentRecords[$key]);
                }
            }
        }
    ...
    protected function getSliderPagesRecords(): array
    {
        $select = $this->pi_prependFieldsWithTable('pages', 'uid,pid,title,subtitle,abstract,media');
        if ($this->extConf['ownMarker_1'] && (strpos($this->extConf['ownMarker_1'], 'pages.') !== false)) {
            $select .= ',' . trim($this->extConf['ownMarker_1']);
        }
        if ($this->extConf['ownMarker_2'] && (strpos($this->extConf['ownMarker_2'], 'pages.') !== false)) {
            $select .= ',' . trim($this->extConf['ownMarker_2']);
        }
        $where = 'pages.uid IN(' . $this->conf['pages'] . ')';
        $where .= $this->cObj->enableFields('pages');
        $rows = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows($select, 'pages', $where, '',
            ' FIELD(uid, ' . $this->conf['pages'] . ')');
        $pageRecords = [];
        foreach ($rows as $key => $row) {
            $pageRecords[$key] = $row;
            $pageRecords[$key]['image'] = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',',
                $pageRecords[$key]['media']);
            $pageRecords[$key]['image'] = $pageRecords[$key]['image']['0'];
            unset($pageRecords[$key]['media']);
            if ($GLOBALS['TSFE']->sys_language_content) {
                $origImg = $pageRecords[$key]['image'];
                $OLmode = $GLOBALS['TSFE']->sys_language_mode == 'strict' ? 'hideNonTranslated' : '';
                $pageRecords[$key] = $GLOBALS['TSFE']->sys_page->getPageOverlay($pageRecords[$key],
                    $GLOBALS['TSFE']->sys_language_content, $OLmode);
                $pageRecords[$key]['image'] = $pageRecords[$key]['image'] ? $pageRecords[$key]['image'] : $origImg;
                if ($GLOBALS['TSFE']->sys_language_mode == 'strict' && !$pageRecords[$key]['_PAGES_OVERLAY']) {
                    $unset = true;
                }
            }
            if ($unset) {
                unset($pageRecords[$key]);
            }
        }
    ...
    protected function getNews(array $sliderRecords = [])
    {
        ...
        foreach ($sliderRecords as $key => $record) {
            ...
            if ($this->conf['sliderStyle'] != 7 && !$record['type']) {
                if (trim($record['tx_rgmediaimages_config'])) {
                    if ($this->conf['videoOverlay']) {
                        $videos = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',',
                    ...
                    if ($this->conf['videoOverlay']) {
                        ...
                        $mmType = \TYPO3\CMS\Core\Utility\GeneralUtility::compat_version('4.7') ? $flexformData['data']['sGeneral']['lDEF']['mmType']['vDEF'] : $this->pi_getFFvalue($flexformData,
                            'mmType');
                        if ($mmType == 'video') {
                            $record['iframe'] = true;
                            $this->iframeOverlay = true;
                            $record['mmFile'] = \TYPO3\CMS\Core\Utility\GeneralUtility::compat_version('4.7') ? $flexformData['data']['sVideo']['lDEF']['mmFile']['vDEF'] : $this->pi_getFFvalue($flexformData,
                                'mmFile');
                            $record['mmFile'] = $this->checkVideo($record['mmFile']);
                            $linkConf['parameter'] = $record['mmFile'];
                            $linkConf['ATagParams'] = 'class="iframe-overlay' . $this->cObj->data['uid'] . '"';
                    ...
                    if ($this->conf['newsOverlay']) {
                        ...
                        if ($GLOBALS['TSFE']->config['config']['prefixLocalAnchors']) {
                            $GLOBALS['TSFE']->config['config']['prefixLocalAnchors'] = '0';
                        }
            ...
            if ($record['author_email']) {
                $marksItem['###AUTHORLINK###'] = $record['author'] ? '<span class="author">' . $this->cObj->typolink($record['author'],
                        $emailLinkConf) . '</span>' : '';
                $marksItem['###AUTHORLINK_ICON###'] = $record['author'] ? '<span class="author_icon">' . $this->cObj->typolink($record['author'],
                        $emailLinkConf) . '</span>' : '';
            }
            ...
            $subpartContentArray['###SLIDER###'] .= $this->cObj->substituteMarkerArrayCached($templateFileItem,
                $marksItem);
            $templateFileItem = $this->cObj->getSubpart($templateFile, '###NAVIGATION###');
            $subpartContentArray['###NAVIGATION###'] .= $this->cObj->substituteMarkerArrayCached($templateFileItem,
                $marksItem);
            // ... pagination-marker
    ...
    protected function getContent(array $sliderRecords = [])
    {
    ...
    protected function getMenu(array $sliderRecords = [])
    {
        // if no template is loaded - use the default template
        ...
        foreach ($sliderRecords as $key => $record) {
            ...
            if ($this->conf['sliderStyle'] == 7 && $this->conf['controlNav'] == 2) {
                unset($thumbConf['stdWrap.']);
                $thumb = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode('"', $this->cObj->IMAGE($thumbConf));
    ...
    protected function getGlobalMarker()
    {
        ...
        if ($this->conf['sliderStyle'] == 2 && !$this->conf['variant']) {
            $this->imageWidth = $this->sliderWidth - $this->conf['navigatorWidth'] + 15;
        }
        if ($this->conf['sliderStyle'] == 3 && !$this->conf['variant']) {
            $this->imageWidth = $this->sliderWidth - $this->conf['navigatorWidth'] + 15;
        }
        if ($this->variant) {
            $this->imageWidth = $this->sliderWidth;
        ...
        if ($this->extConf['manualImageWidth']) {
            $this->imageWidth = intval($this->extConf['manualImageWidth']);
        }
        if ($this->extConf['manualImageHeight']) {
            $this->imageHeight = intval($this->extConf['manualImageHeight']);
        }
        // textarea -setting
        if ($this->conf['textareaWidth']) {
            ...
            if ($this->conf['sliderStyle'] == 5) {
                $this->conf['textareaWidth'] = $this->sliderWidth - 20;
            }
            if ($this->conf['sliderStyle'] == 7) {
                $this->conf['textareaWidth'] = $this->sliderWidth;
            }
            if ($this->variant) {
                $this->conf['textareaWidth'] = 200;
            }
        }
        // thumbnail -setting
        $this->conf['thumbnailSize'] = $this->conf['thumbnailSize'] ? $this->conf['thumbnailSize'] : 60;
        $this->conf['thumbnailWidth'] = $this->inRange($this->conf['thumbnailSize'], 20,
            $this->conf['navigatorHeight'] - 14);
        ...
        if ($this->conf['textareaOffsetBottom']) {
            ...
            if ($this->conf['sliderStyle'] == 5 || $this->conf['sliderStyle'] == 7) {
                $this->conf['textareaOffsetBottom'] = 0;
            }
        }
        if ($this->conf['textareaOffsetLeft']) {
            ...
            if ($this->conf['sliderStyle'] == 3) {
                $this->conf['textareaOffsetLeft'] = 20;
            }
        }
    ...
    {
        $defaultImages = [];
        $defaultImages['files'] = $this->conf['defaultImage'] ? \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',',
            $this->conf['defaultImage']) : '';

        return $defaultImages;
    }

    protected function checkVideo(string $mmFile)
    {
        // check youtube
        if (strpos($mmFile, 'watch?v=') !== false) {
            $record['mmFile'] = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode('watch?v=', $mmFile);
            $record['mmFile'] = 'http://youtube.com/v/' . $record['mmFile'][1];
        }
        // check vimeo
        if (strpos($mmFile, 'vimeo.com') !== false && strpos($mmFile, 'player.vimeo.com') === false) {
            $record['mmFile'] = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode('vimeo.com/', $mmFile);
     ...
     * @return array
     */
    protected function textConversion(array $record = []): array
    {
        ...
        if ($this->conf['contentStyle'] == 'tt_news') {
            ...
            $record['short'] = $record['short'] ? LF . htmlspecialchars($record['short']) : '';
            $record['author_email'] = trim($record['author_email']);
            $record['author_email'] = \TYPO3\CMS\Core\Utility\GeneralUtility::validEmail($record['author_email']) ? $record['author_email'] : '';
        ...
        if ($this->conf['contentStyle'] == 'pages') {
            ...
            $record['abstract'] = $record['abstract'] ? LF . htmlspecialchars($record['abstract']) : '';
        } else {
            if ($this->extConf['rteRendering']) {
                $bodytext_parsehtml_proc = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance(\TYPO3\CMS\Core\Html\RteHtmlParser::class);
    ...
    protected function imageConf(array $record = [], $key, $randbefore, $v)
    {
        ...
        if ($record['CType'] == 'media' || $record['tx_rgmediaimages_config']) {
            if ($this->extConf['videoImages']) {
                $videoImages = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $this->extConf['videoImages']);
                for ($n = 0; $n < count($videoImages); ++$n) {
                    $imageConf['file'] = $videoImages[$v];
                }
                ++$v;
            } else {
                $record['image'] = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $record['image']);
                $imageConf['file'] = $record['image'] ? $uploadDir . $record['image'][0] : '';
            }
        } else {
            // ... only default images - no images from the tt_news-records
            if ($this->conf['requireImage'] == 1) {
                $record['image'] = '';
            }
            $record['image'] = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $record['image']);
            if (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('rgmediaimages')) {
                foreach ($record['image'] as $k => $recimg) {
                    if (strpos($recimg, '.flv') !== false || strpos($recimg, '.pdf') !== false || strpos($recimg,
                            '.ai') !== false || strpos($recimg, '.swf') !== false || strpos($recimg,
                            '.mp3') !== false || strpos($recimg, '.rgg') !== false
                    ) {
                        $record['image'][$k] = '';
                    }
                }
        ...
        if (!$imageConf['file'] || $imageConf['file'] == 'uploads/pics/' || $imageConf['file'] == 'uploads/media/') {
            if ($defaultImages['files']) {
                $uploadDir = $this->noPath ? '' : 'uploads/tx_t3sjslidernews/';
                ...
                if ($imgCount == 1) {
                    ...
                    if ($this->conf['imageOrder']) {
                        ...
                        if ($random == $randbefore) {
                            $random = $random == 1 ? $random + 1 : $random - 1;
                        }
                        $randbefore = $random;
                        $rKey = $random - 1;
        ...
        return [$record, $imageConf, $thumbConf, $randbefore, $v];
    }

    protected function getDefaultImagesDamData($record = [], $defaultImages = [], $rKey)
    {
    ...
    protected function getRecordMarker(array $record = [], array $linkConf = [])
    {
        // title-marker
        ...
        if ($record['short']) {
            ...
            if ($count >= $this->conf['fixedLgd']) {
                ...
                $marksItem['###READMORE###'] = $record['bodytext'] ? $this->cObj->typolink($this->pi_getLL('readmore'),
                    $linkConf) : '';
            }
            ...
            if ($count >= $this->conf['fixedLgd']) {
                $record['bodytext'] = $this->extConf['crop'] ? $this->cObj->cropHTML($record['bodytext'],
                    $fixedLgdpParameter) : $this->cObj->crop(strip_tags($record['bodytext']), $fixedLgdpParameter);
                $marksItem['###READMORE###'] = $this->cObj->typolink($this->pi_getLL('readmore'), $linkConf);
            } else {
                $marksItem['###READMORE###'] = '';
            }
        ...
        if ($GLOBALS['TSFE']->tmpl->setup['config.']['renderCharset'] == 'utf-8') {
            $marksItem['###DATE###'] = $this->extConf['disableUtf8Encode'] ? '<span class="date">' . strftime($this->conf['dateFormat'],
                    $record[$dateField]) . '</span>' : '<span class="date">' . utf8_encode(strftime($this->conf['dateFormat'],
                    $record[$dateField])) . '</span>';
        } else {
            $marksItem['###DATE###'] = '<span class="date">' . strftime($this->conf['dateFormat'],
                    $record[$dateField]) . '</span>';
        }
        $marksItem['###DATE_ICON###'] = '<span class="date_icon">' . strftime($this->conf['dateFormat'],
                $record[$dateField]) . '</span>';
        // category-marker
        if ($this->conf['contentStyle'] == 'tt_news') {
        ...
        if ($this->conf['sliderStyle'] == 6) {
            $marksItem['###DESCRIPTION-STYLE###'] = 'width:' . $this->imageWidth . 'px;';
        }
        if ($this->conf['sliderStyle'] == 2 && $this->conf['variant']) {
            $marksItem['###DESCRIPTION-STYLE###'] = 'width:' . $this->conf['textareaWidth'] . 'px; top:0px; left:0px;';
        }
        if ($this->conf['sliderStyle'] == 3 && $this->conf['variant']) {
            $marksItem['###DESCRIPTION-STYLE###'] = 'width:' . $this->conf['textareaWidth'] . 'px; top:0px; right:0px;';
        }
        ...
        if ($this->conf['sliderStyle'] == 2 && $this->conf['variant']) {
            $marksItem['###NAVSELECTOR-STYLE###'] = 'padding-left:0';
        }
    ...
    protected function getRgnewsce(array $record = []): array
    {
        ...
        $ce_rows = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('uid,header,bodytext,image,CType,pi_flexform,section_frame',
            'tt_content', $where, '', ' FIELD(uid, ' . $record['tx_rgnewsce_ce'] . ')');
        if ($record['type'] == 0 || $record['type'] == 1 || $record['type'] == 2) {
            foreach ($ce_rows as $ce_row) {
                ...
                if (strlen($ce_row['image'])) {
                    $record['image'] = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $ce_row['image']);
    ...
    protected function inRange(int $num, int $min, int $max): int
    {
        $range = range($min, $max);
    ...
    protected function addJquery(\TYPO3\CMS\Core\Page\PageRenderer $pagerender)
    {
        ...
        if ($this->extConf['includeJquery']) {
            ...
            if ($this->extConf['moveJsFromFooterToHeader']) {
                $pagerender->addJsLibrary(' jQuery JavaScript Library ',
                    'https://ajax.googleapis.com/ajax/libs/jquery/' . $version . '/jquery.min.js', 'text/javascript',
                    false, false, '', true);
            } else {
                $pagerender->addJsFooterLibrary(' jQuery JavaScript Library ',
                    'https://ajax.googleapis.com/ajax/libs/jquery/' . $version . '/jquery.min.js', 'text/javascript',
                    true, false, '', true);
            }
        }
    ...
    protected function addHeaderParts(\TYPO3\CMS\Core\Page\PageRenderer $pagerender, $queryNoConflict)
    {
        // jQuery easing
        ...
        if ($this->conf['direction']) {
            $options .= 'direction:\'opacity\',';
        }
        $this->conf['event'] = !$this->conf['event'] ? 'click' : $this->conf['event'];
        if ($this->conf['event'] == 'mouseover') {
            $options .= 'navigatorEvent:\'' . $this->conf['event'] . '\',';
        }
        if ($this->conf['interval'] && $this->conf['interval'] != 4000) {
            $options .= 'interval:' . $this->conf['interval'] . ',';
        }
        if (!$this->conf['auto']) {
            $options .= 'auto:false,';
        }
        if ($this->conf['maxItemDisplay'] != 3) {
            $options .= 'maxItemDisplay:' . $this->conf['maxItemDisplay'] . ',';
        }
        if (($this->conf['sliderStyle'] == 1 || $this->conf['sliderStyle'] == 4) && !$this->conf['verticalNav']) {
            $options .= 'navPosition:\'horizontal\',';
        }
        if ($this->conf['sliderStyle'] == 8) {
            $options .= 'navPosition:\'horizontal\',';
            if ($this->conf['topBottomNav']) {
                $options .= 'topBottom:\'top\',';
            }
        }
        if ($this->conf['navigatorHeight'] != 100) {
            $options .= 'navigatorHeight:' . $this->conf['navigatorHeight'] . ',';
        }
        if ($this->conf['navigatorWidth'] != 310) {
            $options .= 'navigatorWidth:' . $this->conf['navigatorWidth'] . ',';
        }
        if ($this->conf['duration'] && $this->conf['duration'] != 600) {
            $options .= 'duration:' . $this->conf['duration'] . ',';
        }
        $this->conf['easing'] = !$this->conf['easing'] ? 'easeInOutExpo' : $this->conf['easing'];
        if ($this->conf['easing'] != 'easeInOutQuad') {
            $options .= 'easing:\'' . $this->conf['easing'] . '\',';
        }
        if (($this->conf['pause'] == 1) && $this->conf['auto']) {
            $options .= 'pauseOnMouseOver:true,';
        }
        if (($this->conf['pause'] == 2) && $this->conf['auto']) {
            $options .= 'pauseButton:true,';
        }
        if ($this->conf['progressbar']) {
            $options .= 'progressBar:true,';
        }
        if ($this->conf['overlay'] || $this->conf['newsOverlay'] || $this->conf['videoOverlay']) {
            $options .= 'colorbox:true,';
        }
...
function makeShort(){ jQuery(this).animate({\'' . $leftRight . '\':-' . $short . '},200);}';
        ...
        // colorbox-overlay
        if ($this->conf['contentStyle'] != 'pages' && ($this->iframeOverlay || $this->inlineOverlay)) {
            $this->colorboxConf($pagerender, $queryNoConflict);
        }
    }

    protected function addNivoHeaderParts(\TYPO3\CMS\Core\Page\PageRenderer $pagerender, $queryNoConflict)
    {
        // Nivo Slider
        if ($this->extConf['moveJsFromFooterToHeader']) {
        ...
        $options = '';
        if ($this->conf['effect']) {
            $options = 'effect:\'' . $this->conf['effect'] . '\',';
        }
        if ($this->conf['slices']) {
            $options .= 'slices:' . $this->conf['slices'] . ',';
        }
        if ($this->conf['animSpeed']) {
            $options .= 'animSpeed:' . $this->conf['animSpeed'] . ',';
        }
        if ($this->conf['pauseTime']) {
            $options .= 'pauseTime:' . $this->conf['pauseTime'] . ',';
        }
        if ($this->conf['manualAdvance']) {
            $options .= 'manualAdvance:true,';
        }
        if ($this->conf['pauseNivo'] != 1) {
            $options .= 'pauseOnHover:false,';
        }
        if (!$this->conf['directionNav']) {
            $options .= 'directionNav:false,';
        }
        if ($this->conf['controlNav'] == 0) {
            $options .= 'controlNav:false,';
        }
        if ($this->conf['controlNav'] == 2) {
            $options .= 'controlNavThumbs:true,';
        }
        if ($this->conf['boxCols']) {
            $options .= 'boxCols:\'' . $this->conf['boxCols'] . '\',';
        }
        if ($this->conf['boxRows']) {
            $options .= 'boxRows:\'' . $this->conf['boxRows'] . '\',';
        }
    ...
    protected function colorboxConf(\TYPO3\CMS\Core\Page\PageRenderer $pagerender, $queryNoConflict)
    {
        // jQuery colorbox
        ...
        if ($this->extConf['opacity'] != 0.85) {
            $settings = 'opacity:' . $this->extConf['opacity'] . ',';
        }
        if ($this->extConf['transition'] != 'elastic') {
            $settings .= 'transition:\'' . $this->extConf['transition'] . '\',';
        }
        if ($this->extConf['speed'] != 350) {
            $settings .= 'speed:' . $this->extConf['speed'] . ',';
        }
        if ($settings) {
        ...
        if ($this->extConf['includeInlineJS']) {
            ...
            if ($this->extConf['moveInlineJsFromFooterToHeader']) {
                if ($this->inlineOverlay) {
                    $pagerender->addJsInlineCode($name . '- inline', $inlineOverlay);
                }
                if ($this->iframeOverlay) {
                    $pagerender->addJsInlineCode($name . '- iframe', $inlineVideo);
                }
            } else {
                if ($this->inlineOverlay) {
                    $pagerender->addJsFooterInlineCode($name . '- inline', $inlineOverlay);
                }
                if ($this->iframeOverlay) {
                    $pagerender->addJsFooterInlineCode($name . '- iframe', $inlineVideo);
                }
            }
        }
    }
