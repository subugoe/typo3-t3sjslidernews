...
{
    public $prefixId = 'tx_t3sjslidernews_pi1';        // Same as class name
    public $scriptRelPath = 'pi1/class.tx_t3sjslidernews_pi1.php';    // Path to this script relative to the extension dir.
    public $extKey = 't3s_jslidernews';    // The extension key.
  ...
  public $iframeOverlay = false;
    ...
    $newsConf['selectFields'] = $this->pi_prependFieldsWithTable('tt_news', 'uid,pid,title,datetime,image,imagecaption,imagealttext,imagetitletext,short,bodytext,author,author_email,category,type,ext_url,page,sys_language_uid,archivedate');
        ...
        if (t3lib_extMgm::isLoaded('rgmediaimagesttnews')) {
    ...
    $newsRes = $_tt_news_obj->cObj->exec_getQuery('tt_news', $newsConf);
        ...
        if (!$_tt_news_confArr['l10n_mode_imageExclude'] && $newsRecords[$nUid]['_LOCALIZED_UID']) {
            ...
            if ($this->DAM) {
                if ($llRec['tx_damnews_dam_images']) {
                    $newsRecords[$nUid]['llDamImage'] = true;
                }
            } else {
    ...
    protected function getNews($sliderRecords = [])
    {
            ...
            if ($this->conf['sliderStyle'] != 7 && !$record['type']) {
                if (trim($record['tx_rgmediaimages_config'])) {
                    if ($this->conf['videoOverlay']) {
                        $videos = t3lib_div::trimExplode(',', str_replace(["\r\n", "\n", "\r"], ',', $record['tx_rgmediaimages_config']));
                        $record['iframe'] = true;
    ...
    protected function getContent($sliderRecords = [])
    {
    ...
    protected function getMenu($sliderRecords = [])
    {
        // if no template is loaded - use the default template
    ...
    protected function textConversion($record = [])
    {
    ...
    protected function imageConf($record = [], $key, $randbefore, $v)
    {
    ...
    if (!$imageConf['file'] || $imageConf['file'] == 'uploads/pics/' || $imageConf['file'] == 'uploads/media/') {
        if ($defaultImages['files']) {
            ...
            if ($imgCount == 1) {
                ...
                if ($this->DAM) {
                    $record = $this->getDefaultImagesDamData($record, $defaultImages, 0);
                }
            } else {
                if ($this->conf['imageOrder']) {
                    $imageConf['file'] = $uploadDir.$defaultImages['files'][$key];
                    if ($this->DAM) {
                        $record = $this->getDefaultImagesDamData($record, $defaultImages, $key);
                    }
                } else {
                    ...
                    if ($this->DAM) {
                        $record = $this->getDefaultImagesDamData($record, $defaultImages, $rKey);
                    }
                }
            }
        } else {
    ...
    protected function getRecordMarker($record = [], $linkConf = [])
    {
        // title-marker
    ...
    protected function getDamData($record = [], $table, $ref)
    {
        $fields = tx_dam_db::getMetaInfoFieldList();
        $fields .= ',tx_dam.caption,tx_dam.description,tx_dam.abstract,tx_dam.creator,tx_dam.publisher,tx_dam.copyright';
    ...
    protected function getRgnewsce($record = [])
    {
